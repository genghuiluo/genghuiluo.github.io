...
### Dynamic queries with local variables and parameter(s) 
t-sql parameterized query
``` sql
...
``` sql
EXEC AdventureWorks2008.dbo.sp_executeSQL N'exec sp_help'
--数据库中所有对象类型
```
...
### loop through all databases to get table count
``` sql
...
### SQL Server Assemble, Test & Execute Dynamic SQL Command
``` sql
...
EXECUTE sp_executeSQL @SQLStatement -- Execute Dynamic SQL
-- (256 row(s) affected)
```

Dynamic alias for column / table 
``` sql
...
### Automatic code generation for datetime conversion from style 0 to 14
``` sql
...
### Communicating between parent and child process using a shared temporary table
``` sql
...
### Forming proper database object names within a dynamic SQL query stored procedure with QUOTENAME:
``` sql
